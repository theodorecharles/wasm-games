...
assert(['before', 'after'].includes(mode), 'usage: test-bindings-release.mjs before|after');
...
const services = { wolf3d: { port: 8011, candidatePort: 32996, stem: 'wolf', proofStem: 'wolf-bindings-final' },
  spear: { port: 8012, candidatePort: 32997, stem: 'spear', proofStem: 'spear-bindings' } };
const images = {}, http = {}, evidence = {};
for (const [game, config] of Object.entries(services)) {
  const tag = `local/${game}-wasm:keys-context-20260906`;
  ...
  const required = [['menu', 'menu'], ['visible', 'menu'], ['world', 'gameplay'], ['forward', 'gameplay'],
    ['fire', 'gameplay'], ['pause', 'paused'], ['resumed', 'gameplay']];
  // Wolf's follow-up was observed during enemy-death animation, not stable
  // gameplay. Retain that record separately; do not accept it as this gate.
  if (game === 'spear') required.push(['resume-fire', 'gameplay']);
  for (const [suffix, state] of required) {
    const stem = `${config.proofStem}-${suffix}-2026-09-06`;
...
assert.equal(Object.keys(patches).length, 6);
const bindings = JSON.parse(fs.readFileSync(path.join(proofDir, 'key-bindings-native-2026-09-06.json')));
for (const game of Object.keys(services)) {
  assert.equal(bindings.results[game].cases, 161);
  assert.equal(bindings.results[game].failures, 0);
}
for (const file of ['key-bindings-native-2026-09-06.json', 'gameplay-input-sdk-2026-09-06.json'])
  evidence[file] = hash(fs.readFileSync(path.join(proofDir, file)));
...
if (mode === 'before') {
  for (const [game, config] of Object.entries(services)) {
    ...
    const candidate = containers[`${config.stem}-keys-context-proof-20260906`];
  ...
  const before = JSON.parse(fs.readFileSync(path.join(proofDir, 'bindings-release-before-2026-09-06.json')));
  ...
  for (const game of Object.keys(services)) {
    ...
    assert.equal(inspect(`local/${game}-wasm:before-bindings-20260906`).Id, old.image);
  }
