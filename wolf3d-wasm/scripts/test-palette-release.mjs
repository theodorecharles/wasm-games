...
assert(['before', 'after'].includes(mode), 'usage: test-palette-release.mjs before|after');
...
const services = { wolf3d: { port: 8011, candidatePort: 32990, stem: 'wolf' },
  spear: { port: 8012, candidatePort: 32991, stem: 'spear' } };
const images = {}, http = {}, evidence = {};
for (const [game, config] of Object.entries(services)) {
  const tag = `local/${game}-wasm:palette-release-20260906`;
  ...
  for (const [suffix, state] of [['view-visible', 'menu'], ['view-cancel-clean', 'menu'],
    ['sensitivity-visible', 'menu'], ['world', 'gameplay'], ['forward', 'gameplay'],
    ['fire-observed', 'gameplay'], ['pause', 'paused'], ['resumed', 'gameplay']]) {
    const stem = `${config.stem}-palette-${suffix}-2026-09-06`;
...
if (mode === 'before') {
  for (const [game, config] of Object.entries(services)) {
    ...
    const candidate = containers[`${config.stem}-palette-proof-20260906`];
  ...
  const before = JSON.parse(fs.readFileSync(path.join(proofDir, 'palette-release-before-2026-09-06.json')));
  ...
  for (const game of Object.keys(services)) {
    ...
    assert.equal(inspect(`local/${game}-wasm:before-palette-20260906`).Id, old.image);
  }
