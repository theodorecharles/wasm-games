(() => {
  ...
  function startTelemetry(ctx) {
    ...
    stateTimer = window.setInterval(() => {
      ...
      if (typeof engine?._Q1Web_NetworkState === 'function') {
        document.documentElement.dataset.quakeNetwork = [
          engine._Q1Web_NetworkState(),
          engine._Q1Web_NetworkPacketsSent(),
          engine._Q1Web_NetworkPacketsReceived()
        ].join(',');
      }
      if (typeof engine?._Q1_BrowserConnected === 'function') {
        document.documentElement.dataset.quakeConnected =
          String(Boolean(engine._Q1_BrowserConnected()));
        document.documentElement.dataset.quakePose = [
          engine._Q1_BrowserOriginX100() / 100,
          engine._Q1_BrowserOriginY100() / 100,
          engine._Q1_BrowserOriginZ100() / 100,
          engine._Q1_BrowserYawX100() / 100
        ].join(',');
      }
  ...
  globalThis.WasmGameAdapter = Object.freeze({
      ...
      wakeClient = typeof ctx.framework.createWakeClient === 'function'
        ? ctx.framework.createWakeClient({
          ...
          onStatus(status) {
            ...
            document.documentElement.dataset.quakeServerState = String(status.state || 'unknown');
            document.documentElement.dataset.quakeServerPeers = String(status.peers || 0);
            document.documentElement.dataset.quakeServerBots = String(status.bots || 0);
          }
        }) : null;
    ...
    async start(ctx) {
      ...
      const name = launchMode === 'deathmatch'
        ...
        : safeName(ctx.preferences?.values().playerName);
      ...
      if (multiplayerServer) {
        args.push('+connect', String(multiplayerServer.connect || '127.0.0.1:26000'));
        ctx.log('Deathmatch: connecting to the framework-managed FrikBotNex server through the datagram relay.');
      }
