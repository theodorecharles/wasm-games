// Test-only: pair the native q4-intro-decal trace with its next actual GPU draw.
...
OffscreenCanvas.prototype.getContext = function(...args) {
  ...
  for (const method of ['drawElements','drawArrays']) {
    ...
    gl[method]=function(...args) {
      if (!introNext) return draw(...args);
      const label=introNext; introNext=null;
      const result={label,method,args,preexistingErrors:errors(),state:{},attributes:[],uniforms:{}};
      for (const name of ['BLEND','BLEND_SRC_RGB','BLEND_DST_RGB','BLEND_SRC_ALPHA','BLEND_DST_ALPHA','BLEND_EQUATION_RGB','DEPTH_TEST','DEPTH_FUNC','DEPTH_WRITEMASK','DEPTH_RANGE','COLOR_WRITEMASK','CULL_FACE','CULL_FACE_MODE','FRONT_FACE','SCISSOR_TEST','SCISSOR_BOX','VIEWPORT']) result.state[name]=plain(get(gl[name]));
      ...
      introPost({type:'log',text:'[q4-intro-draw] '+JSON.stringify(result)});
      return value;
    };
  }
  return gl;
