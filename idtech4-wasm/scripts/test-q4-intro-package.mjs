...
for(const [name,port,draw] of [['q4-intro-trace-proof-20260906',32958,false],['q4-intro-draw-proof-20260906',32959,true]]) {
  ...
  if(draw) {
    assert.equal(actual[worker],hash(fs.readFileSync(path.join(root,'tests/q4-intro-draw-worker.js'))));
...
for(const p of ['tests/q4-intro-decal-diagnostic.patch','tests/q4-intro-draw-worker.js','patches/openq4-browser.patch']) result.sources[p]=hash(fs.readFileSync(path.join(root,p)));
...
const proof=path.join(root,'proofs/quake4-intro-package-2026-09-06.json');
...
console.log('Both isolated observers verified; 45/44 unchanged installed files, exact HTTP bytes, read-only owner files, and restored canonical source/build.');
