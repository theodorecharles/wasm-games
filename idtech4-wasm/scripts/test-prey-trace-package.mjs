...
const name = 'prey-trace-cache-proof-20260906';
const candidate = inspect(name), old = inspect('prey-menu-cache-proof-20260905');
const prior = JSON.parse(fs.readFileSync(path.join(root, 'proofs/prey-save-package-2026-09-06.json')));
...
assert.equal(inspect('local/idtech4-wasm:prey-trace-cache-candidate').Id, candidate.Image);
...
assert.ok(changed.includes('/opt/game-site/prey06.wasm'));
assert.ok(changed.includes('/opt/game-site/game-adapter.js'));
assert.ok(changed.every(file => ['/opt/game-site/prey06.js', '/opt/game-site/prey06.wasm', '/opt/game-site/game-adapter.js'].includes(file)));
...
const sourceProof = JSON.parse(fs.readFileSync(path.join(root, 'proofs/prey-trace-cache-source-2026-09-06.json')));
assert.deepEqual(source, sourceProof);
const regression = JSON.parse(fs.readFileSync(path.join(root, 'proofs/prey-trace-cache-native-2026-09-06.json')));
assert.equal(regression.clipSHA256, source.files['neo/game/physics/Clip.cpp']);
assert.equal(regression.headerSHA256, source.files['neo/game/physics/Clip.h']);
assert.equal(regression.saveSHA256, source.files['neo/game/gamesys/SaveGame.cpp']);
assert.equal(regression.fixtureSHA256, hash(fs.readFileSync(path.join(root, 'tests/d3-trace-cache.cpp'))));
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'prey-trace-package-'));
function copy(file) {
...
const result = {
  scope:'Isolated Prey ownership repair and existing shared-adapter integration. Exact source/regression/native/HTTP identity, unchanged remaining package files and complete installed six-variant adapter suite. Chrome save compatibility is separate; no live promotion.',
  ...
  unchanged:Object.keys(files).length - changed.length, files, source, ownerFiles:13,
  adapterFixture:{status:fixture.status, stdout:fixture.stdout, stderr:fixture.stderr}
};
const proof = path.join(root, 'proofs/prey-trace-cache-package-2026-09-06.json');
...
console.log(`Prey repaired source/native/HTTP identity, ${result.unchanged} unchanged files, six installed variants and 13 read-only owner files verified.`);
