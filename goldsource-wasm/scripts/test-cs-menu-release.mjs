...
const oldImage = 'sha256:967561eca15081284748f7a46178a1f5d8f8024a3aa182e4a95e2a406ca9c2ce';
const newImage = 'sha256:3ac53414f7fbfb8ec82ec79ecd9efb8759bae71934bfd21585267dab411821b4';
const liveName = '/wasm-goldsource-suite';
const candidateName = '/goldsource-cs-menu-globals-chrome-proof-20260906';
...
const earlier = await readJSON(path.join(proofs, 'goldsource-release-installed-2026-09-06.json'));
const baseline = installed ? await readJSON(path.join(proofs, 'cs-menu-globals-before-2026-09-06.json')) : earlier;
...
const oldMenu = '/opt/game-site/artifacts/cs-menu-framework-QC5WOJ5I.wasm';
const newMenu = '/opt/game-site/artifacts/cs-menu-framework-YQVL64O7.wasm';
...
assert.equal(after[newMenu].sha256, 'aa34b47854c6321cc3a3bd7241edc1c09eb3ecd63753f827f7a6e62f94713867');
...
const accepted = await readJSON(path.join(proofs, 'cs-menu-globals-package-2026-09-06.json'));
const origin = installed ? 'http://127.0.0.1:8017' : 'http://127.0.0.1:32946';
const http = [];
...
const report = { observedAt: new Date().toISOString(), installed, oldImage, newImage, containers: current,
  ...
  unchangedContainers: installed ? current.length - 1 : earlier.containers.length - 1, passed: true };
await fs.writeFile(path.join(proofs, `cs-menu-globals-${installed ? 'installed' : 'before'}-2026-09-06.json`),
