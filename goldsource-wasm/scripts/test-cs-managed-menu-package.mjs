#!/usr/bin/env node
// Compare the managed-menu candidate with the prior global-isolation release.
...
const candidate = process.env.CS_MENU_CANDIDATE || 'http://127.0.0.1:32948';
const baseline = process.env.CS_MENU_BASELINE_ORIGIN || 'http://127.0.0.1:32946';
...
for (const file of await files(path.join(root, 'web'))) {
  ...
  if (file === newMenu) {
    ...
    const orderedExports = module => WebAssembly.Module.exports(module).sort((a, b) => a.name.localeCompare(b.name));
    assert.deepEqual(orderedExports(newModule), [...orderedExports(oldModule),
      { name: '_ZN9CMenuMain4ShowEv', kind: 'function' }].sort((a, b) => a.name.localeCompare(b.name)),
      'every prior export is preserved; only the new main-menu Show override is added');
    assert(!WebAssembly.Module.exports(newModule).some(item => item.name === 'gpGlobals'));
...
const report = { testedAt: new Date().toISOString(), candidate, baseline, unchanged, changed, readiness,
  priorExportSetPreserved: true, addedExport: '_ZN9CMenuMain4ShowEv', menuGlobalsRemainPrivate: true,
  gameplayAcceptance: false, passed: true };
