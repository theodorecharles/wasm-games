...
const oldOrigin=process.env.OPENRCT2_OLD_ORIGIN || 'http://127.0.0.1:32937';
const candidateOrigin=process.env.OPENRCT2_CANDIDATE_ORIGIN || 'http://127.0.0.1:32938';
const baseline=JSON.parse(await fs.readFile(path.join(proofs,'rct1-sprite-path-package-2026-09-05.json')));
...
assert.ok(patches.some(p=>p.name.endsWith('/construction-ghost-identity.patch')));
assert.ok(patches.some(p=>p.name.endsWith('/browser-dialog-capability.patch')));
const report={observedAt:new Date().toISOString(),oldOrigin,candidateOrigin,sourceCommit:head,patches,files,data,
  acceptance:'Exact public/staged/native artifacts and private boundaries only; compiled source regressions, linker diagnostics and Chrome behavior are separate.'};
if(process.argv[2]) await fs.writeFile(process.argv[2],JSON.stringify(report,null,2)+'\n');
console.log('OpenRCT2 construction/dialog package: exact 19-file staging and prior baseline, only native JS/Wasm changed, same pinned source and private installation, private-boundary checks pass.');
